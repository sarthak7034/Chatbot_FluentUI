...
export const detectPerformanceIssues = (): {
  ...
  const slowRenders = metricsHistory.filter(m => m.renderTime > slowRenderThreshold).length;
  
  // Simple memory leak detection
  const recentMetrics = metricsHistory.slice(-10);
  const memoryTrend = recentMetrics.length > 1 ? 
  ...
  if (memoryLeaks) {
    recommendations.push('Potential memory leak detected - check for uncleared intervals/timeouts');
